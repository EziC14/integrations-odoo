...
namespace OdooCls.API.Controllers
{
    ...
    [ApiController]
    public class RegistroComprasController : ControllerBase
    {
        private readonly IConfiguration configuration;
        private readonly IRegistroComprasRepository registro;
        private readonly RegistroComprasServices registroComprasApplication;
        string? keys = "";
        private string VALID_TOKEN = "";


        public RegistroComprasController(IConfiguration configuration,IRegistroComprasRepository registro)
        {
            this.configuration = configuration;
            this.registro = registro;
            registroComprasApplication = new RegistroComprasServices(this.configuration, this.registro);
        ...
        [HttpPost]
        [Route("create")]
        public async Task<IActionResult> CreateRegComp([FromBody] RegistroComprasDto rcdto)
        {
            try
            {
                ...
                var response = await registroComprasApplication.CreateRegComAsync(rcdto);
            ...
            catch (Exception ex)
            {
                ...
                var errorResponse = new ApiResponse<RegistroComprasDto>(500, 500, $"Error interno del servidor: {ex.Message}");
